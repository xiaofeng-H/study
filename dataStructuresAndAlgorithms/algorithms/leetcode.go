...
func isSameTree100(p *TreeNode, q *TreeNode) bool {
	// 边界值处理（注意以下两个if条件的并列使用：先与后或---nbnb）
	// 都为空的话，显然相同
	if p == nil && q == nil {
		return true
	}
...
func deleteNode(root *TreeNode, key int) *TreeNode {
	...
	if root.Val == key {
		...
		// 处理情况3：左右子树均不空
		// 此处使用右子树的最小值替换掉原来结点的值，然后在右子树中删除其最小值（回到情况1）
...
func countNodes222(root *TreeNode) int {
	...
	// 如果左右子树的高度相同，则是一颗满二叉树（卧槽NB 2023/10/13 17:14）
	if hl == hr {
...
func MinWindow76(s string, t string) string {
	...
	left, right := 0, 0
	// valid 变量表示窗口中满足 need 条件的字符个数，如果 valid 和 len(need) 的大小相同，则说明窗口已满足条件，已经完全覆盖了串 t
	valid := 0
	// 记录最小覆盖子串的起始索引及长度
...
/*====================================== BFS start ============================================*/
/**
注意：在用 BFS/DFS 遍历时，若是树的遍历，则不需要记录已经遍历过的结点，若为图的遍历，则需要记录已经遍历过的结点防止走回头路！！！
*/
/*
「力扣」第 111 题（二叉树的最小深度）
EASY!!!直接套 BFS 框架就完事了。
...
*/
/*
再再读有感：（2023/10/28 11:55）
第一眼看到这个问题，想着既然要求从 “0000” 拨动到 “xxxx” 的最少拨动次数，那我每一位从 0 开始，如果该位置的目标值小于等于 5，
则从 0 开始由小到大依次拨动到目标值，若大于 5，则从 9 开始从大到小拨动到目标值，每一位都做此操作，不就是最少的拨动次数嘛！！！
想到这种特殊情况，终于明白了题目中 deadends 这组死亡数字的作用了。若在 deadends 数组中设置一对死亡序列，其满足任意三位与目标值相同，
不同的一位大于和小于目标值中余下的一位，这样死亡序列就可以顺利截胡了，上面那种拨法就会失效，因为会撞到死亡数字，导致所做的努力白费。
哈哈哈，题目中原来真的没有废话，惜字如金呀！！！
*/
// 将s[j]向上拨动一次
...
著作权归领扣网络所有。商业转载请联系官方授权，非商业转载请注明出处。
*/
// 解法一：之前牛客的题解，使用的是哈希表的思想(不要求原始数组有序)(nb:2023/10/16 16:43)
...
func TwoSum2(nums []int, target int) []int {
	...
	for left < right {
		...
		if sum == target {
			...
			// 当两数之和小于目标值时，因为 right 已经是当前最大值不能再增加，所以只能让 left 右移以逼近目标值
			// 让sum大一点
			left++
		} else if sum > target {
			// 同理，当两数之和大于目标值时，因为 left 已经是当前最小值不能再减小，所以只能让 right 左移以逼近目标值
			// 让sum小一点
			right--
		}
	}
...
/*
「力扣」第 206 题（反转链表）（超级超级经典算法）（超级超级基础算法）
*/
/**
...
*/
/**
一看这归并写得就不太行，想必是当时按照自己的理解写的吧！（用一个 for 循环直接搞定）（2023/10/28 16:44）
更为经典的归并写法详见：study/dataStructuresAndAlgorithms/algorithms/sort/sort_merge.go:102
*/
