...
/*
回文链表（力扣234）
算法思想：借助⼆叉树后序遍历的思路，不需要显式反转原始链表也可以倒序遍历链表，即
链表的后序遍历。不过使用该方法的空间复杂度和反转链表的空间复杂度是一样的，都为
O(N)。使用快慢指针可优化空间复杂度，详解看代码。
时间复杂度：O(N)
空间复杂度：O(1)
*/
...
/*
被围绕的区域（力扣130）
算法思想：常规解法是DFS（虽然我不会），此处使用并查集即：只有和边界  O  相连的  O  才具有和  dummy  的连通性，他们不会被替换。
时间复杂度：
空间复杂度：
*/
