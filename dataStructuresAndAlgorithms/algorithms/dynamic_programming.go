...
时间复杂度：O(n)
空间复杂度：O(5*n)
参考链接：https://zhuanlan.zhihu.com/p/357397457
*/
func maxProfit123(prices []int) int {
	/* 题解：
	   怎么感觉可以用我解决Ⅱ用的代码呀！
	   eee，其实是不一样的。
		自己戳破脑袋想了三天三夜，一直困扰在Ⅱ的思路中无法自拔，早已忘却了这是动规的题目，或许是因为对于Ⅱ的题解颇为得意吧！
	其实Ⅱ也不是官解，只是自己蒙出来的，既不是贪心，亦不是动规，属于是二不象了。
		无奈，只得求助其他高人，看了别人的状态定义之后，大受震撼，世间竟还有如此思路清奇之人，五体投地都不足以表达本人的钦佩之情。
	eee，自己上手之后才发现，并非高人有多高，实为自己有多菜。说到底，动规不就是用空间换时间么！那就只剩一句话了：菜就多练！！！
	*/
	/*
		第一步：确定状态
			每一天都有五个操作，即动规中的状态，分别为无为--0、第一次买入--1、第一次卖出--2、第二次买入--3、第二次卖出--4等五个状态。
		第二步；确定dp数组含义
			dp[i][j]：第i+1天做完j操作后所拥有的最大现金。
		第三步：明确初始值
			dp[0][0] = 0：不做任何操作自然没有金钱流入流出；
			dp[0][1] = -prices[0]：买入则亏
			dp[0][2] = 0：无买入，卖出无利润
			dp[0][3] = -prices[0]：买入则亏
			dp[0][4] = 0：无买入，卖出无利润
		第四步：动态规划择优
			取最大值
	*/
